...
import { INITIAL_USERS, INITIAL_STOCKS, INITIAL_REMINDERS, INITIAL_NOTIFICATIONS, INITIAL_ACTIVITIES, DEFAULT_SETTINGS, INITIAL_TODOS } from '../mock-data/initialData';
...
export const StateProvider = ({ children }) => {
    ...
    const [currentUser, setCurrentUserInternal] = useState(() => {
    ...
    const logActivity = (action, category, details) => {
        const newLog = {
    ...
    const executeStockIn = async (id, qty, comments) => {
        setIsLoading(true);
        await new Promise((r) => setTimeout(r, 500));
        let updatedItemName = '';
        let currentQty = 0;
    ...
    const executeStockOut = async (id, qty, comments) => {
        setIsLoading(true);
        await new Promise((r) => setTimeout(r, 500));
        let updatedItemName = '';
        let isSuccess = true;
