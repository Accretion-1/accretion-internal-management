...
import { Phone, ShieldAlert, ArrowRight, CheckCircle2, Building2, HelpCircle } from 'lucide-react';
...
export const LoginPage: React.FC = () => {
  ...
  const [smsIncomingAlert, setSmsIncomingAlert] = useState<{show: boolean; text: string} | null>(null);
  ...
  const proceedToOTPStep = () => {
    ...
    setStep('otp');
    // Display simulated SMS Toast
    setSmsIncomingAlert({
      show: true,
      text: `[SIMULATED SMS] WorkSphere verification code: 123456. Valid for 5 minutes.`
    });
  };
  ...
  const handleOtpSubmit = async (e: React.FormEvent) => {
    ...
    const result = await login(phoneNumber, otpValue);
    if (!result.success) {
  ...
  const handleQuickAutofill = (phone: string) => {
  ...
  return (
    <div className="relative min-h-screen bg-slate-50 flex flex-col justify-between overflow-hidden font-sans selection:bg-blue-100">
            ...
            <div className="mt-4 p-5 bg-white border border-slate-200 rounded-2xl shadow-sm">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                <button
                  ...
                  onClick={() => handleQuickAutofill('9999999991')}
                  className="p-2.5 rounded-xl border border-slate-100 hover:border-blue-200 bg-slate-50 hover:bg-blue-50/50 text-left transition-all hover:scale-101 cursor-pointer"
                >
                ...
                <button
                  id="sandbox-manager-btn"
                  onClick={() => handleQuickAutofill('9999999992')}
                  className="p-2.5 rounded-xl border border-slate-100 hover:border-blue-200 bg-slate-50 hover:bg-blue-50/50 text-left transition-all hover:scale-101 cursor-pointer"
                >
                ...
                <button
                  id="sandbox-user-btn"
                  onClick={() => handleQuickAutofill('9999999993')}
                  className="p-2.5 rounded-xl border border-slate-100 hover:border-blue-200 bg-slate-50 hover:bg-blue-50/50 text-left transition-all hover:scale-101 cursor-pointer"
                >
                  ...
                  >
                    ...
                    <form onSubmit={handlePhoneSubmit} className="flex flex-col gap-4">
                      <div className="flex flex-col gap-1.5">
                        ...
                        <div className="relative flex items-center">
                          ...
                          <input
                            ...
                            className={`w-full pl-12 pr-4 py-3.5 bg-slate-50 border ${phoneError ? 'border-rose-400 focus:ring-rose-200' : 'border-slate-200 focus:ring-blue-200'} rounded-2xl text-slate-900 placeholder-slate-400 font-medium focus:outline-none focus:ring-4 focus:bg-white transition-all text-sm`}
                          />
                        </div>
                        {phoneError ? (
                          ...
                          <span className="text-[11px] text-slate-400">Sandbox config requires registered demo accounts.</span>
                        )}
                      </div>
                      ...
                      <button
                        ...
                        type="submit"
                        disabled={isLoading}
                        className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3.5 rounded-2xl font-semibold shadow-md shadow-blue-100 hover:shadow-lg transition-all flex items-center justify-center gap-2 cursor-pointer text-sm"
                      >
                        Send OTP Code
                    ...
                    <form onSubmit={handleOtpSubmit} className="flex flex-col gap-4">
                      <div className="flex flex-col gap-1.5">
                        ...
                        <input
                          ...
                          className={`w-full px-4 py-3.5 text-center text-xl tracking-widest bg-slate-50 border ${otpError ? 'border-rose-400' : 'border-slate-200'} rounded-2xl text-slate-900 font-bold focus:outline-none focus:ring-4 focus:ring-blue-100 focus:bg-white transition-all`}
                        />
                        {otpError && (
                          <div className="flex items-center gap-1.5 text-xs text-rose-600 mt-1">
                            <ShieldAlert className="w-3.5 h-3.5" />
                      ...
                      <button
                        ...
                        type="submit"
                        disabled={isLoading}
                        className="w-full bg-slate-900 hover:bg-slate-850 text-white py-3.5 rounded-2xl font-semibold hover:shadow-lg transition-all flex items-center justify-center gap-2 cursor-pointer text-sm"
                      >
                      ...
                      <button
                        ...
                        className="text-xs font-semibold text-slate-500 hover:text-slate-800 text-center mt-1 cursor-pointer transition-colors"
                      >
                        Modify phone number
                      </button>
                    </form>
      ...
      {/* Simulated SMS Push notification layer to assist evaluation */}
      <AnimatePresence>
        {isSimulatingSMS && (
          <div className="fixed inset-0 bg-slate-900/40 z-50 flex items-center justify-center p-4">
            <motion.div
              ...
              className="bg-white rounded-2xl p-6 max-w-sm w-full border border-slate-200 shadow-2xl flex flex-col gap-4 text-center"
            >
              <div className="w-12 h-12 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center mx-auto">
                <CheckCircle2 className="w-6 h-6" />
              </div>
              <div>
                <h4 className="font-display font-bold text-slate-900 text-md">Simulating SMS Delivery</h4>
                <p className="text-xs text-slate-500 mt-1 leading-normal">
                  In production, an automated API dispatches a 6-digit passcode. Tap below to auto-fetch the verification SMS.
                </p>
              </div>
              <button
                id="sandbox-sms-trigger"
                onClick={proceedToOTPStep}
                className="w-full bg-slate-900 text-white font-medium text-xs py-3 rounded-xl hover:bg-slate-800 transition-all cursor-pointer"
              >
                Fetch Verification SMS (123456)
              </button>
            </motion.div>
          </div>
        )}
      ...
      <AnimatePresence>
        {smsIncomingAlert?.show && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 20 }}
            className="fixed bottom-6 inset-x-6 sm:left-auto sm:right-6 max-w-sm bg-slate-900 text-slate-100 rounded-2xl shadow-xl overflow-hidden z-40 border border-slate-800 pointer-events-auto"
          >
