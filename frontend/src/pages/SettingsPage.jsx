...
import { Settings, Building2, KeyRound } from 'lucide-react';
export const SettingsPage = () => {
    ...
    return (<div className="flex flex-col gap-6 font-sans text-left pb-12 max-w-4xl mx-auto">
          ...
          <form onSubmit={handleSaveSecurity} className="bg-white border border-slate-200/80 shadow-xs rounded-2xl p-6 flex flex-col gap-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-semibold text-slate-750">
              <div className="flex flex-col gap-1.5 text-left">
                ...
                <input type="text" disabled={!isAdmin} value={orgName} onChange={(e) => setOrgName(e.target.value)} className="px-4 py-2.5 bg-slate-50 disabled:bg-slate-100/60 border border-slate-200 rounded-xl focus:outline-none focus:ring-4 focus:ring-blue-105 focus:bg-white text-xs font-semibold text-slate-800"/>
              </div>

              <div className="flex flex-col gap-1.5 text-left">
                <label className="text-xs">System Safety Protocol Level</label>
                <select disabled={!isAdmin} value={safetyLevel} onChange={(e) => setSafetyLevel(e.target.value)} className="px-4 py-2.5 bg-slate-50 disabled:bg-slate-100/60 border border-slate-200 rounded-xl focus:outline-none text-xs font-semibold text-slate-800 cursor-pointer">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-semibold text-slate-750 pt-1">
              <div className="flex flex-col gap-1.5 text-left">
                ...
                <input type="text" disabled={!isAdmin} value={ipRanges} onChange={(e) => setIpRanges(e.target.value)} placeholder="e.g. 192.168.0.0/16" className="px-4 py-2.5 bg-slate-50 disabled:bg-slate-100/60 border border-slate-200 rounded-xl focus:outline-none focus:ring-4 focus:ring-blue-105 focus:bg-white text-xs font-mono font-bold text-slate-800"/>
              </div>
              ...
              <div className="flex flex-col gap-2 text-left justify-center">
                <label className="flex items-center gap-2 cursor-pointer select-none font-bold text-slate-705">
                  <input type="checkbox" disabled={!isAdmin} checked={mfa} onChange={(e) => setMfa(e.target.checked)} className="rounded border-slate-300 text-blue-600 focus:ring-blue-500 w-4 h-4 cursor-pointer"/>
            ...
            <div className="flex items-center justify-between border-t border-slate-100 pt-4 mt-1">
              ...
              {isAdmin && (<button id="save-settings-btn" type="submit" disabled={isLoading} className="px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-xl text-xs transition-colors cursor-pointer">
          ...
          <div className="bg-white border border-slate-200/80 shadow-xs rounded-2xl p-6 flex flex-col gap-5 text-left">
            ...
            <div className="flex flex-col gap-1 text-xs text-slate-505 font-medium select-none">
              ...
              <div className="flex flex-wrap gap-1.5 mt-2">
                {currentUser?.assignedModules.map((m) => (<span key={m} className="px-2 py-0.5 bg-slate-100 text-slate-600 rounded text-[10px] font-bold">
                    {m}
                  </span>))}
              </div>
            </div>
