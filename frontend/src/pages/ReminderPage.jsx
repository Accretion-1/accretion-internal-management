...
import { Clock, Plus, Calendar, ClipboardList, UserPlus, Trash, BellRing } from 'lucide-react';
import { Modal } from '../components/Modal';
export const ReminderPage = () => {
    const { currentUser, users, reminders, createReminder, snoozeReminder, completeReminder, deleteReminder, isLoading } = useAppState();
    ...
    return (<div className="flex flex-col gap-6 font-sans text-left pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        {isAdminOrManager && (<button id="schedule-reminder-btn" onClick={handleOpenCreate} className="inline-flex items-center gap-2 px-4.5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-semibold shadow-xs hover:shadow-md cursor-pointer transition-all shrink-0">
      ...
      <div className="flex justify-between items-center border-b border-slate-200 overflow-x-auto hide-scrollbar">
        <div className="flex gap-4 p-1 whitespace-nowrap">
          {['Active', 'Snoozed', 'Completed', 'All'].map((tab) => (<button key={tab} id={`reminder-tab-${tab}`} onClick={() => setActiveTab(tab)} className={`px-4 py-2.5 text-xs font-bold border-b-2 transition-all cursor-pointer ${activeTab === tab
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {filteredReminders.length > 0 ? (filteredReminders.map((rem) => {
            return (<div key={rem.id} className="bg-white rounded-3xl border border-slate-200/80 shadow-xs p-6 flex flex-col justify-between gap-6 transition-all hover:shadow-md hover:scale-[1.01]">
                ...
                <div className="flex flex-col gap-4 border-t border-slate-100 pt-4 text-xs font-medium">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    return (<span key={userId} className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-slate-50 text-slate-750 border border-slate-200/60 rounded-xl text-[11px] font-semibold">
                  ...
                  <div className="flex sm:items-center justify-between flex-col sm:flex-row gap-3 pt-1">
                    ...
                    {rem.status !== 'Completed' && (<div className="flex items-center gap-1.5 self-end">
                        
                        {/* Snooze button (only for Active items) */}
                        {rem.status === 'Active' && (<button id={`snooze-reminder-${rem.id}`} onClick={() => snoozeReminder(rem.id)} className="px-3 py-1.5 text-slate-650 hover:bg-slate-100 rounded-lg cursor-pointer border border-slate-200 transition-colors font-semibold text-[11px]">
                            Snooze 30M
                          </button>)}

                        {/* Complete button */}
                        <button id={`complete-reminder-${rem.id}`} onClick={() => completeReminder(rem.id)} className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg cursor-pointer shadow-xs transition-colors font-bold text-[11px]">
                          Resolve Duty
                        </button>
                    ...
                    {isAdminOrManager && rem.status === 'Completed' && (<button id={`delete-reminder-${rem.id}`} onClick={() => deleteReminder(rem.id)} className="px-3 py-1.5 text-rose-500 hover:bg-rose-50 rounded-lg cursor-pointer transition-colors font-bold text-[11px] inline-flex items-center gap-1 border border-rose-100 ml-auto">
      ...
      <Modal isOpen={isCreateOpen} onClose={() => setIsCreateOpen(false)} title="Schedule WorkSphere Duty Reminder" footerButtons={[
        ...
        <div className="flex flex-col gap-4 text-left">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input id="rem-create-title" type="text" placeholder="e.g. OSHA Safety Certificate Submission" value={title} onChange={(e) => setTitle(e.target.value)} className="px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-4 focus:ring-blue-50 focus:bg-white text-xs font-semibold text-slate-800"/>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <textarea id="rem-create-desc" rows={3} placeholder="Detailed guidelines describing key duties to perform..." value={description} onChange={(e) => setDescription(e.target.value)} className="px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-4 focus:ring-blue-50 focus:bg-white text-xs font-medium text-slate-800 resize-none"/>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-semibold text-slate-755">Limit Date</label>
              <input id="rem-create-date" type="date" value={dueDate} onChange={(e) => setDueDate(e.target.value)} className="px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none text-xs font-semibold text-slate-805"/>
            </div>

            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-semibold text-slate-755">Limit Time</label>
              <input id="rem-create-time" type="time" value={dueTime} onChange={(e) => setDueTime(e.target.value)} className="px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none text-xs font-semibold text-slate-805"/>
            </div>
          </div>

          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-semibold text-slate-755">Recurrence Routine</label>
            <select id="rem-create-recurrence" value={recurrence} onChange={(e) => setRecurrence(e.target.value)} className="px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none text-xs font-semibold text-slate-800 cursor-pointer">
          ...
          <div className="flex flex-col gap-2.5 mt-1">
            ...
            return (<label key={u.id} className="flex items-center gap-2 text-xs font-semibold text-slate-650 cursor-pointer select-none">
                    <input type="checkbox" checked={checked} onChange={() => handleToggleAssignee(u.id)} className="rounded border-slate-300 text-blue-600 focus:ring-blue-500"/>
                    {u.name}
                  </label>);
        })}
            </div>
