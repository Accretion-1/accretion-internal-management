...
import { 
  Package, Search, PlusCircle, ArrowDownCircle, ArrowUpCircle, 
  AlertTriangle, Filter, ClipboardList, RefreshCw, Layers, MapPin 
} from 'lucide-react';
import { Modal } from '../components/Modal';

export const StockPage: React.FC = () => {
  const { 
    currentUser, stocks, executeStockIn, executeStockOut, createStockItem, isLoading, showToast 
  } = useAppState();
  ...
  const createSkuItem = async (): Promise<boolean> => {
    const qtyVal = parseInt(skuQty);
    const minVal = parseInt(skuThreshold);
  ...
  return (
    <div className="flex flex-col gap-6 font-sans text-left pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="w-full bg-slate-100 rounded-full h-3">
            <div 
              className={`h-3 rounded-full transition-all duration-500 ${
                capA_percent >= 85 ? 'bg-rose-500' : capA_percent >= 60 ? 'bg-amber-500' : 'bg-blue-600'
              }`} 
              style={{ width: `${capA_percent}%` }} 
            />
          </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs text-left">
          ...
          <div className="w-full bg-slate-100 rounded-full h-3">
            <div 
              className={`h-3 rounded-full transition-all duration-500 ${
                capB_percent >= 85 ? 'bg-rose-500' : capB_percent >= 60 ? 'bg-amber-500' : 'bg-amber-500'
              }`} 
              style={{ width: `${capB_percent}%` }} 
            />
          </div>
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col gap-4">
        <div className="flex flex-col lg:flex-row gap-3">
          ...
          <div className="relative flex-1">
            ...
            <input
              id="sku-search-filter"
              type="text"
              placeholder="Search assets inside stock count catalog by Name, Category, or SKU..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-205 rounded-xl text-xs placeholder-slate-400 font-medium focus:outline-none focus:ring-2 focus:ring-blue-105 focus:bg-white transition-all text-slate-800"
            />
          </div>
        ...
        {filteredStocks.length > 0 ? (
          filteredStocks.map((item) => {
            ...
            return (
              <div 
                key={item.id} 
                className={`bg-white rounded-3xl border shadow-xs p-6 flex flex-col justify-between gap-5 transition-all hover:shadow-md hover:scale-[1.01] ${
                  isLowStock ? 'border-rose-200/80' : 'border-slate-200/80'
                }`}
              >
                ...
                <div className="flex justify-between items-start">
                  ...
                  {isLowStock ? (
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[9px] uppercase font-extrabold bg-rose-50 border border-rose-100 text-rose-600 animate-pulse">
                      Low warning
                    </span>
                  ) : (
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[9px] uppercase font-semibold bg-emerald-50 border border-emerald-100 text-emerald-600">
                      Standard
                    </span>
                  )}
                </div>
                ...
                <div className="flex flex-col gap-3.5 border-t border-slate-100 pt-3 text-xs">
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-1">
                    ...
                    <button
                      ...
                      className="px-3 py-2 bg-slate-50 hover:bg-slate-100 text-slate-700 font-semibold border border-slate-200 shadow-xs rounded-xl cursor-pointer flex items-center justify-center gap-1.5 transition-colors text-xs"
                    >
                    ...
                    <button
                      ...
                      className="px-3 py-2 bg-slate-50 hover:bg-slate-100 text-slate-705 font-semibold border border-slate-200 shadow-xs rounded-xl cursor-pointer flex items-center justify-center gap-1.5 transition-colors text-xs"
                    >
      ...
      <Modal
        ...
        title={`Increase Stock Reserves: ${selectedItem?.name}`}
        footerButtons={[
        ...
        {selectedItem && (
          <div className="flex flex-col gap-4 text-left">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input
                id="action-stock-in-comments"
                type="text"
                placeholder="e.g. Incoming shipment from Vendor PL-998"
                value={txComments}
                onChange={(e) => setTxComments(e.target.value)}
                className="px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-4 focus:ring-blue-50 focus:bg-white text-xs font-medium text-slate-800"
              />
            </div>
          </div>
        )}
      ...
      <Modal
        ...
        title={`Decrease Stock Reserves: ${selectedItem?.name}`}
        footerButtons={[
        ...
        {selectedItem && (
          <div className="flex flex-col gap-4 text-left">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input
                id="action-stock-out-comments"
                type="text"
                placeholder="e.g. Dispatched to Shift-B team for pallet installation"
                value={txComments}
                onChange={(e) => setTxComments(e.target.value)}
                className="px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-4 focus:ring-blue-50 focus:bg-white text-xs font-medium text-slate-800"
              />
      ...
      <Modal
        ...
        title="Inventory New SKU Catalogue Asset"
        footerButtons={[
        ...
        <div className="flex flex-col gap-4 text-left">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="flex flex-col gap-1.5">
              ...
              <input
                id="sku-create-name"
                type="text"
                placeholder="e.g. Structural Steel Bar C"
                value={skuName}
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input
                id="sku-create-code"
                type="text"
                placeholder="e.g. BAR-STL-808"
                value={skuCode}
                onChange={(e) => setSkuCode(e.target.value)}
                className="px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-4 focus:ring-blue-50 focus:bg-white text-xs font-mono font-bold text-slate-850"
              />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <select
                id="sku-create-warehouse"
                value={skuWarehouse}
                onChange={(e) => setSkuWarehouse(e.target.value)}
                className="px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none cursor-pointer text-xs font-semibold text-slate-800"
              >
                {WAREHOUSES.map((wh) => (
                  <option key={wh} value={wh}>{wh}</option>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input
                id="sku-create-threshold"
                type="number"
                placeholder="e.g. 20"
                value={skuThreshold}
                onChange={(e) => setSkuThreshold(e.target.value)}
                className="px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-4 focus:ring-blue-50 focus:bg-white text-xs font-semibold text-slate-800"
              />
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <select
                ...
                className="px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none cursor-pointer text-xs font-semibold text-slate-850"
              >
